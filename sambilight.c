...
typedef struct {
    char host[50];
    unsigned long port;
    unsigned short led_start; 
    unsigned short led_end;   
} WLED_Device;

WLED_Device wled_devices[MAX_WLED_DEVICES];
unsigned char num_wled_devices = 0;
	...
	while (1) {
		if (led_manager_get_state()) {
			...
			if (led_manager_argb8888_to_leds(buffer, &data[header_size])) {
				for (int i = 0; i < num_wled_devices; i++) {
					unsigned short segment_start = wled_devices[i].led_start;
					unsigned short segment_end = wled_devices[i].led_end;
					size_t segment_led_count = segment_end - segment_start + 1;
					size_t segment_data_size = header_size + (segment_led_count * 3) + (external_led_enabled ? 1 : 0);

					unsigned char segment_header[6];
					memcpy(segment_header, header, header_size);
					segment_header[2] = (segment_led_count * 3) >> 8;
					segment_header[3] = (segment_led_count * 3) & 0xff;
					uint8_t *segment_led_data = data + header_size + (segment_start * 3);

					uint8_t *send_buffer = malloc(segment_data_size);
					if (!send_buffer)
						continue;

					memcpy(send_buffer, segment_header, header_size);
					memcpy(send_buffer + header_size, segment_led_data, segment_led_count * 3);
					if (external_led_enabled)
						send_buffer[segment_data_size - 1] = external_led_state;

					struct sockaddr_in wled_addr;
					memset(&wled_addr, 0, sizeof(wled_addr));
					wled_addr.sin_family = AF_INET;
					wled_addr.sin_port = htons(wled_devices[i].port);
					wled_addr.sin_addr.s_addr = inet_addr(wled_devices[i].host);

					sendto(sockfd, (const char *)send_buffer, segment_data_size, 0,
						   (struct sockaddr *)&wled_addr, sizeof(wled_addr));
...
EXTERN_C void lib_init(void* _h, const char* libpath)
{
	...
	char opt_key[20];
	for (int i = 1; i <= MAX_WLED_DEVICES; i++) {
		snprintf(opt_key, sizeof(opt_key), "HOST%d:", i);
		optstr = getOptArg(argv, argc, opt_key);
		if (optstr && strlen(optstr)) {
			strncpy(wled_devices[num_wled_devices].host, optstr, sizeof(wled_devices[num_wled_devices].host));

			snprintf(opt_key, sizeof(opt_key), "PORT%d:", i);
			optstr = getOptArg(argv, argc, opt_key);
			wled_devices[num_wled_devices].port = (optstr && strlen(optstr)) ? atol(optstr) : port;

			snprintf(opt_key, sizeof(opt_key), "LED_START%d:", i);
			optstr = getOptArg(argv, argc, opt_key);
			wled_devices[num_wled_devices].led_start = (optstr && strlen(optstr)) ? atoi(optstr) : 0;

			snprintf(opt_key, sizeof(opt_key), "LED_END%d:", i);
			optstr = getOptArg(argv, argc, opt_key);
			wled_devices[num_wled_devices].led_end = (optstr && strlen(optstr)) ? atoi(optstr) : led_manager_get_leds_count() - 1;
	...
	log("SamyGO mod DMagliarisi 2025\n");
        log("[Sambilight] %d WLED devices configured\n", num_wled_devices);
